...
public abstract class AbstractSnippet extends AbstractEntryPoint implements
		IViewerMenuSupport {
	private IEditingDomainProvider editingDomainProvider;
	private EditorForm editorForm;
	private CreateChildrenActionContributor createChildActionContributor = new CreateChildrenActionContributor();
	...
	protected void createContents(Composite parent) {
		...
		parent.getDisplay().disposeExec(new Runnable() {
			...
			public void run() {
				...
				FrameworkUtil.getBundle(getClass()).getBundleContext()
						.ungetService(modelSetRef);
			}
		});
		parent.setLayout(new FillLayout());
		editingDomainProvider = (IEditingDomainProvider) modelSet.adapters()
				...
				if (selection instanceof IStructuredSelection) {
					...
					if (first instanceof IReference) {
						...
						RWT.getClient()
								...
								.execute(
										"(window.parent || window).postMessage(\"{ selection : '"
												+ first + "' }\", '*')");
					}
				}
				createChildActionContributor.selectionChanged(null,
		...
		if (modelUri != null) {
			try {
				IModel model = modelSet.getModel(URIs.createURI(modelUri),
						false);
				if (model != null) {
	...
	public void createContextMenuFor(StructuredViewer viewer) {
		...
		contextMenu.addMenuListener(new IMenuListener() {
			...
			public void menuAboutToShow(IMenuManager manager) {
				...
				createChildActionContributor.menuAboutToShow(manager,
						"additions");
			}
		});
		...
		viewer.addDropSupport(
				dndOperations,
				transfers,
				new EditingDomainViewerDropAdapter(editingDomainProvider
