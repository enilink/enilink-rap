			...
			if (file != null) {
				return new UseService<IModelSet, String>(IModelSet.class) {
					...
					protected String withService(final IModelSet modelSet) {
						...
						InputStream in = null;
						try {
							in = modelSet.getURIConverter().createInputStream(
									modelUri);
							String ontology = ModelUtil.findOntology(
									new FileInputStream(file), modelUri
											.toString(),
									ModelUtil.mimeType(ModelUtil
											.contentDescription(
							...
							InputDialog dialog = new InputDialog(shell,
									"Name the new model.",
									"Please enter an URI for your model.",
									modelUri.toString(), new IInputValidator() {
										@Override
										public String isValid(String newText) {
											...
											IModel existing = modelSet
													.getModel(newURI, false);
											if (existing != null) {
						...
						if (!modelUri.equals(resourceUri)) {
							mapRule = new SimpleURIMapRule(modelUri.toString(),
						...
						try {
							...
							modelSet.createModel(modelUri).load(
