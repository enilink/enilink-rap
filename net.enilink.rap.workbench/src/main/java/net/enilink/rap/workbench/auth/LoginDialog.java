 ...
 *******************************************************************************/
package net.enilink.rap.workbench.auth;
...
public class LoginDialog extends TitleAreaDialog implements CallbackHandler {
	...
	protected Control createDialogArea(Composite parent) {
		...
		if (!loginMethods.isEmpty()) {
			...
			loginMethods.setLayoutData(new GridData(SWT.FILL, SWT.BEGINNING,
					true, false));
		}

		Composite handlerWidgets = new Composite(dialogArea, SWT.NONE);
		handlerWidgets.setLayout(new GridLayout());
		handlerWidgets.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true,
				true));
			...
			methodButton.addSelectionListener(new SelectionAdapter() {
				...
				public void widgetSelected(SelectionEvent e) {
					if (methodButton.getSelection()
							&& !method.equals(selectedMethod)) {
						setLoginData(CURRENT_METHOD_KEY, method);
						getShell().getDisplay().setData(CURRENT_METHOD_KEY,
								method);
						isCancelled = true;
						close();
					}
	...
	private void createPasswordHandler(Composite composite,
		...
		passwordText.setLayoutData(new GridData(SWT.FILL, SWT.BEGINNING, true,
				false));
	...
	private void createTextInputHandler(Composite composite,
		...
		if (callback.getPrompt().toLowerCase().contains("openid")) {
			...
			rememberButton.addSelectionListener(new SelectionAdapter() {
				...
				public void widgetSelected(SelectionEvent e) {
					...
					setLoginData(attributeKey, remember[0] ? text.getText()
							: null);
				}
			});
	...
	private boolean handleNonUserCallbacks() throws IOException {
		...
		for (int i = 0; i < callbacks.length; i++) {
			...
			if (callback instanceof RedirectCallback) {
				...
				((RealmCallback) callback).setContextUrl(getHostAndPath(RWT
						.getRequest()));
				((RealmCallback) callback).setApplicationUrl(contextUrl
						.toString());
				return true;
			} else if (callback instanceof ResponseCallback) {
				((ResponseCallback) callback).setResponseParameters(RWT
	...
	public void create() {
		...
		if (imageDescData instanceof ImageDescriptor) {
			final Image titleImage = ((ImageDescriptor) imageDescData)
					.createImage(false, display);
			if (titleImage != null) {
	...
	protected void openWithEventLoop() throws IOException {
		...
		display.syncExec(new Runnable() {
			public void run() {
				...
				if (!loginMethods.isEmpty()) {
					selectedMethod = (String) display
							.getData(CURRENT_METHOD_KEY);
					String methodFromCookie = getLoginData(CURRENT_METHOD_KEY);
					if (methodFromCookie != null
							&& !methodFromCookie.equals(selectedMethod)
							&& loginMethods.contains(methodFromCookie)) {
					...
					while (shell != null && !shell.isDisposed()) {
						try {
							...
							if (e instanceof ThreadDeath) {
								display.setData("login.exception.threaddeath",
										e);
							}
	...
	private void redirectBrowser(final RedirectCallback callback)
			throws IOException {
		RWT.getClient()
				.getService(JavaScriptExecutor.class)
				.execute(
						"parent.window.location.href=\""
								+ callback.getRedirectTo() + "\";");
		openWithEventLoop();
	}
