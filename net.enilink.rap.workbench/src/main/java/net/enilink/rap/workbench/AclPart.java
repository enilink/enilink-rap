...
public class AclPart extends AbstractEditingDomainPart {
	...
	private static URI[] accessModes = { WEBACL.MODE_READ, WEBACL.MODE_WRITE,
			WEBACL.MODE_CONTROL };
	...
	public void createContents(Composite parent) {
		...
		targetLabel.setLayoutData(new GridData(SWT.FILL, SWT.BEGINNING, true,
				false));
		...
		ownerViewer = new PropertyViewer(section, SWT.SINGLE,
				getWidgetFactory());
		...
		Table table = getWidgetFactory().createTable(section,
		...
		agentColumn.setEditingSupport(new PropertyCellEditingSupport(viewer) {
			...
			protected IStatement getStatement(Object element) {
				return new Statement((IEntity) element, ((IEntity) element)
						.getEntityManager().find(WEBACL.PROPERTY_AGENT,
								IProperty.class), ((Authorization) element)
						.getAclAgent());
			}
		...
		TableViewerColumn agentClassColumn = new TableViewerColumn(viewer,
				SWT.LEFT);
		agentClassColumn.getColumn().setText("AgentClass");
		agentClassColumn.setEditingSupport(new PropertyCellEditingSupport(
				viewer) {
			@Override
			protected IStatement getStatement(Object element) {
				return new Statement((IEntity) element, ((IEntity) element)
						.getEntityManager().find(WEBACL.PROPERTY_AGENTCLASS,
								IProperty.class), ((Authorization) element)
						.getAclAgentClass());
			}
		...
		for (final URI mode : accessModes) {
			TableViewerColumn modeColumn = new TableViewerColumn(viewer,
					SWT.LEFT);
			modeColumn.getColumn().setText(mode.localPart());
			modeColumn.setEditingSupport(new EditingSupport(viewer) {
				@Override
				protected void setValue(Object element, Object value) {
					Class modeClass = ((IEntity) element).getEntityManager()
							.find(mode, Class.class);
					Set<Class> aclModes = ((Authorization) element)
							.getAclMode();
				...
				protected Object getValue(Object element) {
					return ((Authorization) element).getAclMode()
							.contains(mode);
				}
	...
	protected void createActions() {
			...
			public void run() {
				executeCommand(new SimpleCommand() {
					...
					protected CommandResult doExecuteWithResult(
						...
						Authorization auth = target.getEntityManager().create(
								Authorization.class);
						auth.setAclAccessTo((IResource) target);
						auth.getAclMode().add(
								target.getEntityManager().find(
										WEBACL.MODE_READ, Class.class));
						return CommandResult.newOKCommandResult(auth);
					}
				});
			}
		};
		addAction.setImageDescriptor(ExtendedImageRegistry.getInstance()
				.getImageDescriptor(
		...
		deleteAction = new Action("Remove") {
			public void run() {
				ICommand deleteCommand = DeleteCommand.create(
						getEditingDomain(), ((IStructuredSelection) viewer
								.getSelection()).getFirstElement());
				executeCommand(deleteCommand);
			}
		};
		deleteAction.setImageDescriptor(ExtendedImageRegistry.getInstance()
				.getImageDescriptor(
						KommaEditUIPropertiesPlugin.INSTANCE
	...
	public void refresh() {
		...
		if (adapterFactory == null || !adapterFactory.equals(newAdapterFactory)) {
			...
			ownerViewer.setLabelProvider(new AdapterFactoryLabelProvider(
					adapterFactory));
			viewer.setLabelProvider(new AdapterFactoryLabelProvider(
				...
				Object getTarget(Object object, int columnIndex) {
					Authorization authorization = ((IEntity) object)
				...
				public Image getColumnImage(Object object, int columnIndex) {
					if (columnIndex >= MODES_OFFSET) {
						if (((IEntity) object)
								...
								.getAclMode()
								.contains(
										accessModes[columnIndex - MODES_OFFSET])) {
		...
		if (target == null) {
			...
			targetLabel.setImage(((ILabelProvider) viewer.getLabelProvider())
					.getImage(target));
			targetLabel.setText(((ILabelProvider) viewer.getLabelProvider())
					.getText(target));
			targetLabel.getParent().layout(true);
			ownerViewer.setEditingSupport(new ResourceEditingSupport(
					getAdapterFactory()));
			ownerViewer.setInput(new StatementPattern(target,
					WEBACL.PROPERTY_OWNER, null));
			viewer.setInput(new StatementPattern(null,
					WEBACL.PROPERTY_ACCESSTO, target));
			...
			boolean canControl = secureTarget != null
					&& (SecurityUtil.getUser().equals(
							secureTarget.getAclOwner()) || secureTarget
							.hasAclMode(SecurityUtil.getUser(),
									WEBACL.MODE_CONTROL));
