			...
			if (element instanceof IModel) {
				...
				if (model != null) {
					...
					if (MessageDialog.openConfirm(shell, "Delete model",
							"Are you sure to delete model '" + model.getURI()
									+ "'?")) {
						IModelSet modelSet = model.getModelSet();
						IDataChangeSupport changeSupport = model.getModelSet()
								.getDataChangeSupport();
						try {
						...
						MessageDialog.openInformation(shell, "Model deleted",
								"Model '" + model.getURI()
