...
import net.enilink.rap.workbench.auth.LoginDialog;
...
public class Application implements IApplication {
	...
	private static Map<String, String> jaasConfigurations = new LinkedHashMap<String, String>();
	...
	private static final boolean REQUIRE_LOGIN = false
			|| "true".equalsIgnoreCase(System.getProperty("enilink.loginrequired"));

	static {
		try {
			System.setProperty("java.security.krb5.conf",
					new File(FileLocator
							.resolve(Platform.getBundle("net.enilink.core").getResource(KERBEROS_CONFIG_FILE)).toURI())
									.getAbsolutePath());
		} catch (Exception e) {
			// ignore
		}
	...
	public Object start(IApplicationContext context) throws Exception {
		...
		ContextProvider.getApplicationContext().getPhaseListenerManager()
				.addPhaseListener(new UnitOfWorkPhaseListener());
		...
		if (loginRequired && subject == null) {
			...
			ImageDescriptor titleImageDesc = AbstractUIPlugin.imageDescriptorFromPlugin("net.enilink.rap.workbench",
					"/img/enilink/login_title.png");
			...
			while (null == subject && retry) {
				...
				if (loginState == null || !loginMethod.equals(loginState.method)) {
					...
					URL cfgUrl = Platform.getBundle("net.enilink.core").getResource(JAAS_CONFIG_FILE);
				...
				try {
					...
					if (!Boolean.TRUE.equals(display.getData("login.canceled"))) {
						...
						IStatus status = new Status(IStatus.ERROR, EnilinkWorkbenchPlugin.INSTANCE.getSymbolicName(),
								e.getMessage());
	...
	protected int runWorkbench(Display display) {
		WorkbenchAdvisor advisor = new WorkbenchAdvisor() {
			...
			public String getInitialWindowPerspectiveId() {
				return "net.enilink.rap.workbench.basicPerspective";
			}
		};
