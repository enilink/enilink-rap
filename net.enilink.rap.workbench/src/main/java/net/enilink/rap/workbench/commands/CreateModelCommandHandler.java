package net.enilink.rap.workbench.commands;
...
import net.enilink.rap.workbench.EnilinkWorkbenchPlugin;
import net.enilink.rap.workbench.ModelEditorInput;
			...
			return new UseService<IModelSet, String>(IModelSet.class) {
				...
				protected String withService(final IModelSet modelSet) {
					...
					InputDialog dialog = new InputDialog(shell,
							"Name the new model.",
							"Please enter a URI for your model.",
							isAdmin ? modelNS.toString() : "",
							new IInputValidator() {
								@Override
								public String isValid(String newText) {
									try {
										if (isAdmin) {
											...
											newURI[0] = modelNS.trimSegments(1)
													.appendSegments(segments);
										}
									} catch (Exception e) {
										return "The given URI is invalid: "
												+ e.getMessage();
									}
									if (newURI[0].isRelative()) {
										return "The URI must be absolute.";
									}
									IModel existing = modelSet.getModel(
											newURI[0], false);
									if (existing != null) {
						...
						protected Text createText(Composite composite) {
							if (!isAdmin) {
								Composite group = new Composite(composite,
										SWT.NONE);
					...
					if (dialog.open() == Window.OK) {
						...
						// model
						model.getManager().assignTypes(model.getOntology(),
								Ontology.class);
						// open the recently created model
						try {
							IWorkbenchPage page = PlatformUI.getWorkbench()
									.getActiveWorkbenchWindow().getActivePage();
							page.openEditor(new ModelEditorInput(model),
									"net.enilink.rap.workbench.modelEditor",
